...
`started → submitted → under_review → needs_more_information → approved` (with variant `opened` for deposit accounts) `→ declined`, formalized as a state machine with named transition events, plus proposed terminal extensions (`expired`, `withdrawn`). Vendor statuses are mapped at the adapter boundary and never leak past it.
...
**3. Native onboarding SDKs.** iOS and Android components for account-opening and credit-intake flows: a configurable flow engine with step sequencing, theming/branding, validation rules, disclosure and consent capture, accessibility-ready components (WCAG 2.1 AA), analytics hooks, and PII minimized in memory.
...
- **Specification:** v0.1 draft, open for comment. Expect breaking changes between 0.x minors; changes will be recorded in a changelog.
- **Reference implementations:** in progress — an iOS SDK and a Spring Boot middle layer. Android SDK is planned.
- No production adoption is claimed for v0.1; the draft is published to gather implementer feedback first.

## Roadmap

1. **Specification** *(current)* — stabilize the v0.1 taxonomy, API contract, and adapter interface through public comment.
...
Apache-2.0 (placeholder pending initial release; see `LICENSE` when published).
